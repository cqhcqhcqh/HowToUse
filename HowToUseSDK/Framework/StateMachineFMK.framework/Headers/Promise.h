...
typedef void (^resolved_block)(id responseCode);
typedef void (^rejected_block)(NSError *error);
typedef void (^any_block)(void);
typedef void (^pending_Block)(id pending);
...
- (Promise *)when:(resolved_block)whenBlock failed:(rejected_block)rejectedBlock pending:(pending_Block)pendingBlock;

- (Promise *)on:(dispatch_queue_t)queue;
- (Promise *)onMainQueue;

+ (Promise *)timeout:(NSTimeInterval)interval;
+ (Promise *)timeout:(NSTimeInterval)interval withErrorDomin:(NSString *)errorDomin code:(int)errorCode userInfo:(id)userInfo;;
